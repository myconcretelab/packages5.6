...
function ccmValidateBlockForm() {
    if ($('#fsID').val() <= 0 || $('#fsID').val() == null) {
	$('#fsID').css('border','3px solid #ff0000');
        ccm_addError(ccm_t('fsID-required'));
    }

    if ($('#globalImagePID').val() == 0 || $('#globalImagePID').val() == null) {
	$('#globalImagePID').css('border','3px solid #ff0000');
        ccm_addError(ccm_t('globalImagePID-required'));
    }
    return false;
}
...
var refreshPresetsList = function () {
        select = $('select.presets');
	$.ajax({
            url: GET_PRESETS_OPTIONS_URL,
            dataType: 'html',
            success:
                function(response) {
    ...
    //console.log(response);
                        ...
                        if ((($this.attr('title') > 0 || $this.attr('title') == -1 ) && $this.val() == 0 )) {
                            val = $this.attr('title');
                        } else {
                            // L'utilisateur a déjà choisi une valeur (différente que celle défnie par la balise titre)
                            val = $this.val();                            
                        }
                        $this.html(response);
			$this.append('<option value="-1">Random</option>');
                        $this.val(val);
                    })
                }
        });
...
var openFormPreset = function (presetID) {
    pID = (presetID == undefined) ? 0 : presetID;
    if (presetID == -1 ) {
	// Random value
	$('#preset_form').html('<strong>Random is not editable!</strong>');
    } else if (presetID == 0) {
	// -- Chose a Fileset --
	...
	$.ajax({
		url: GET_PRESETS_FORM_URL,
                data:{'pID':pID},
		dataType: 'html',
		success: function(response) {
			$('#preset_form').html(response);
		}
	});
}
...
var savePreset = function (form) {
        ...
        var str = $('#ajax_presets_form').serialize();
        $.ajax({
                 type: "POST",
                 url: GET_PRESETS_SAVE_URL,
                 data: str,
                 success: function(msg){
                    refreshPresetsList(0);
                    $('#preset_form').html('<strong>Preset saved !</strong>');
                 }
        });
}
var deletePreset = function (pID) {
        $.ajax({
                 type: "POST",
                 url: GET_PRESETS_DELETE_URL,
                 data: {'pID':pID},
                 success: function(msg){
                    refreshPresetsList(0);
                    $('#preset_form').html('<strong>Preset deleted !</strong>');

                 }
        });
}
var setjQuerySlider = function ($name,$min,$max,$value,$step,$default_point) {
    $default_point = ( $default_point == undefined) ? 0 : $default_point ;
    $value = ($value == 'default') ? $default_point : $value;
    $step = ($step == undefined) ? 1 : $step;
     ...
     $('#range-'+$name).slider({
		...
		slidejQuerySLider(ui.value,$name, $default_point);
	    }
    });
     slidejQuerySLider($value, $name, $default_point);    
}

var slidejQuerySLider = function ($v, $n, $dp) {
    if ($v > $dp) {
	$('input#'+$n).val($v);	
    } else {
	$('input#'+$n).val('default' );			    
...
function refreshImagesList () {
    ...
    $.ajax({
        type: "POST",
        url: GET_IMAGES_OPTIONS_URL,
        data: {'bID':BLOCK_ID,'fsID':fsID},
        success: function(msg){
           refreshPresetsList();
            $('#ccm-file-infos-list').html(msg);
        }
    });        
}
