...
var setjQuerySlider = function ($name,$min,$max,$value,$step,$default_point) {
    $default_point = ( $default_point == undefined) ? -1 : $default_point ;

   if ($value == 'default') { $value = $default_point } else if ($value == '') {$value = $min};
...
function loadBlockInfos  (stID) {
	...
	$.ajax({
		...
		dataType: 'html',
		success: function(response) {
			$('#ccm-file-infos-list').html(response);
		}
	});
}
