function setjQuerySlider  ($name,$min,$max,$value,$step,$auto_point) {
    $auto_point = ( $auto_point == undefined) ? -20 : $auto_point ;
    $value = ($value == 'auto') ? $auto_point : $value;
    //$value = ($value == '') ? $max / 2 : $value;
    $step = ($step == undefined) ? 1 : $step;
     ...
     $('#range-'+$name).slider({
		...
		slidejQuerySLider(ui.value,$name, $auto_point);
	    }
    });
     slidejQuerySLider($value, $name, $auto_point);    
}

function slidejQuerySLider  ($v, $n, $auto) {
    if ($v > $auto) {
	$('input#'+$n).val($v);	
    } else {
	$('input#'+$n).val('auto' );			    
