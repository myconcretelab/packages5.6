...
class OptionsGeneratorHelper {
	...
	function __construct ($options = null, $saved = null, $action = null,$hiddens = array(), $pID = null) {
		...
		if ($options) : // Option n'est pas rempli quand il est appellŽ par loader::helper
			...
			$this->pID = $pID;
			$this->generator = Loader::helper('base_options_generator', 'theme_super_mint');
	...
	function renderOption($option){
		// On ajoute les option en cousteau suisse
		$option['poh'] = $this->poh;
		$option['pID'] = $this->pID;
		if(isset($option['id'])){
		...
		if (method_exists($this->generator, $option['type'])) { 
			echo '<tr><td style="padding:30px 10px; width:20%"><strong><label for="'.$option['id'].'">' . $option['name'] . '</label></strong><td>';
	...
	function open ($item) {
		echo Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper($item['name'], false, 'span12', true);		
	}
	function start ($item) {
		echo Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper($item['name'], false, 'span12', false);
		echo '<div class="ccm-pane-body">';
		echo '<table cellspacing="0" class="entry-form zebra-striped" style="width:100%;">';
		echo '<tbody>';		
	...
	function section ($item) {
		echo '<tr><td colspan="2" style="padding:30px 0"><h3 style="text-align:center">' . $item['name'] . '</h3></td></tr>';		
	}
