...
$edit_product = Loader::helper('concrete/urls')->getToolsURL('quick_edit_product','theme_super_mint') ;
$cp = new Permissions(Page::getCurrentPage());
$auth = $cp->canAdminPage() && $o->quick_edit_product;
...
if(count($products)>0) : 
	...
	foreach ($products as $i => $product) :
		...
		print '<div class="' . $span . ' pr_simple pr_simple_' . $args['imagePosition'] . '">';
			Loader::element('product/display', $args, 'theme_super_mint');
			if ($auth) : ?>
				<br>
				<a href="<?php echo $edit_product ?>?action=activate&value=0&pID=<?php echo $product->getProductID() ?>" class="ajax">Desactiver</a> / 
				<a href="/dashboard/core_commerce/products/search/edit/<?php echo $product->getProductID() ?>" target="_blank">Editer</a> / 
				<a href="/dashboard/core_commerce/products/images/<?php echo $product->getProductID() ?>" target="_blank">Images</a>
			<?php endif;
		print '</div>';
