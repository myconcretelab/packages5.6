function ccmValidateBlockForm() {
	...
	ccm_addError('You must choose a Fileset');
	$('#fsID').css('border','3px solid red');
	$('.ui-accordion-header:eq(0) a').css('color','red');
    }
    $('.type').each(function() {
	$select = $('select', this);
	if ($select.val() == 0 ) {
	    $select.css('border','3px solid red');
	    $('.ui-accordion-header:eq(1) a').css('color','red');
	    ccm_addError('You must select a Preset or remove it.');
	}
    });
    if ($('#resize').val() == undefined ) {$('#resize').val('no_resize')};
        return false;
}
...
var  refreshFilesetList = function (select_value) {
	...
	$.ajax({
		    ...
		    console.log(response);
                    select.each(function() {
                        ...
                        $this.html(response);
			$this.append('<option value="-1">Default</option>');
                        $this.val(val);
                    })
                }
        });
...
var openFormPreset = function (presetID) {
    ...
    if (presetID == -1 ) {
	// Random vaule
	$('#preset_form').html('<strong>Default is not editable!</strong>');
	return false;
    } else if (presetID == 0) {
	// -- Chose a Fileset --
	return false;	
...
var addAnimationType = function () {
	$('#array_types .type:first-child').clone().children('select').val('0').parent().appendTo('#array_types');
