...
//-----------------  On Window Loaded  ----------------//
// ---------------------------------------------------//
jQuery(window).load(function () { // Load est utilisŽ car sinon la largeur des polices ne correspond pas.
...
jQuery(document).ready(function () {
    ...
    // VŽrifier si la sidebar est remplie. Si pas lui ajouter un contenu transparent evitant l'Žcrasement de sa largeur
...
function make_shadow() {
    jQuery('.bottom-shadow, .bottom-shadow-transparent, .fly-shadow, .fly-shadow-transparent').each(function () {
        ...
        var $parent = $this.parent(); // servira a verifier si le parent ˆ un z-index
        ...
        } else if ($this.children().length == 1 && $this.children().is('img')) { // Si on a entourŽ l'image d'un div (qui n'est pas ˆ la bonne taille)
